...
exports.postMessage = async (req, res, next) => {
  ...
  if (type === "image") imagePath = req.body.imgPath;
  if (type === "audio") audioPath = req.body.audioPath;
...
exports.postSetReceivedMessages = async (req, res, next) => {
  ...
  try {
    ...
    const result = await Message.updateMany(
      {
        conversationId: mongoose.Types.ObjectId(convo._id),
        by: mongoose.Types.ObjectId(sender),
        messageStatus: "SENT",
      },
...
exports.postSetSeenMessages = async (req, res, next) => {
  ...
  try {
    const result = await Message.updateMany(
      {
        conversationId: mongoose.Types.ObjectId(convoId),
        by: mongoose.Types.ObjectId(sender),
      },
      { messageStatus: "SEEN" }
    );
...
exports.deleteChat = async (req, res, next) => {
  ...
  try {
    ...
    await User.updateOne(
      { firebaseUserId: firebaseUserId },
      { $pull: { conversations: { conversationId: convoId } } }
    );

    console.log(guestId + "guestId");
    const tempGuest = await User.findOne(
      { _id: guestId },
      { conversations: 1 }
    );
    ...
    if (
      tempGuest &&
      !tempGuest.conversations.some((convo) => convo.conversationId === convoId)
    ) {
      const r2 = await Convo.deleteOne({
        members: { $all: [host._id, guestId] },
      });
    }
...
exports.deleteMessage = async (req, res, next) => {
  ...
  try {
    ...
    if (code === "dfe") {
      response = await Message.updateOne(
        { uuid: uuid },
        { $push: { deletedFor: { $each: [host._id, guestId] } } }
      );
    }

    response = await Message.updateOne(
      { uuid: uuid },
      { $push: { deletedFor: host._id } }
    );

    if (convo.lastMessage.uuid === uuid) {
      const messages = await Message.find(
        { conversationId: convoId, deletedFor: { $ne: host._id } },
        { _id: 1 }
      );
      const resp = await Convo.updateOne(
        { _id: convoId },
...
exports.postConvo = async (req, res, next) => {
  try {
    ...
    const convo = await Convo.findOne({
      members: { $all: [host._id, guest._id] },
    })
      .populate({ path: "members lastMessage" })
      .exec();
    ...
    if (convo) {
      ...
      const selectedC = await Convo.findOne({
        members: { $all: [host._id, guest._id] },
      })
      ...
      await User.updateOne(
        ...
        {
          $push: {
            conversations: { conversationId: resp._id, unseenCount: 0 },
          },
        }
      );
      await User.updateOne(
      ...
      await User.updateOne(
        { contactNumber: number },
        { $addToSet: { connections: host._id } }
      );
      await User.updateOne(
        { firebaseUserId: userId },
        { $addToSet: { connections: guest._id } }
      );
...
exports.postIncrementUnseenCount = async (req, res, next) => {
  try {
    ...
    const guest = await User.updateOne(
      { firebaseUserId: userId, "conversations.conversationId": convoId },
      {
