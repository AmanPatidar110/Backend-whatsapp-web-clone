...
exports.postStatus = async (req, res, next) => {
  ...
  imagePath = req.body.statusImagePath;
...
exports.getStatusList = async (req, res, next) => {
  try {
    ...
    const host = await User.findOne(
      { firebaseUserId: firebaseUserId },
      { _id: 1, connections: 1 }
    ).populate({ path: "connections", select: "name" });

    const statusPromises = host.toObject().connections.map(async (userId) => {
      const statusOfUser = await Status.find({
        postedBy: userId._id,
        expiry: { $gt: new Date() },
        seenBy: { $ne: host._id },
      });
    ...
    const viewedStatusPromises = host
      .toObject()
    ...
    viewedStatusList = viewedStatusList.filter(
      (single) => single.statusArray.length > 0
    );
    console.log(viewedStatusList);

    const allMyStatus = await Status.find({
      postedBy: host._id,
      expiry: { $gt: new Date() },
    });
